// euler11.cs - Project Euler problem 11: Largest product in a grid
...
internal static class Grid {
    internal static int[,] FromText(string text)
    {
        ...
        var rows = from line
                   in text.Split(new string[] { Environment.NewLine },
        ...
        foreach (var row in rows) {
            Contract.Assert(row.Length == width, "grid must be rectangular");
...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48";
    ...
    private const bool FlipGrid = false; // for testing purposes
    ...
    private static int[,] GetGrid() // if grid dimensions are changed,
    {                               // change assertion accordingly
        var grid = Grid.FromText(GridText);
        Contract.Assert(grid.Height() == 20 && grid.Width() == 20);
#pragma warning disable 0162        // unreachable code
        if (FlipGrid) ApplyHorizontalFlip(grid);
#pragma warning restore
    ...
    private static void Main(string[] args)
    {
        ...
        var adjacent_size = Parse(args, 1, adjacent_bound) ?? 4;
