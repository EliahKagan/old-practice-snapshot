...
internal static class CircularPrimes {
    ...
    private static int CountRotations(int val)
            => Rotations(val).Distinct().Count();
    ...
    internal static IOrderedEnumerable<int> Compute(int n)
            => Arith.Sieve(n).GroupBy(MinRotation)
                             .Where(g => g.Count() == CountRotations(g.Key))
                             .SelectMany(g => g)
                             .OrderBy(p => p);
...
internal static class Solution {
    private static void Main()
    {
        ...
        WriteLine(CircularPrimes.Compute().TakeWhile(p => p < n).Sum());
    }
}
