...
PRIMES = tuple(sieve(10**6)) # to factorize numbers up to 10**12

def max_prime_factor(n):
    if n < 2:
        raise ValueError("can't factorize a number less than 2")
    ...
    return best if n == 1 else n

def getval():
    return int(input())

for _ in range(getval()):
    print(max_prime_factor(getval()))
