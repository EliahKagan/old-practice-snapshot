...
class Frame:
    ...
    __slots__ = ('_junctions', '_nei_iter')
    ...
    def __init__(self, junctions, grid, i, j):
        ...
        self._junctions = (junctions + 1 if len(nei_list) > 1 else junctions)
        self._nei_iter = iter(nei_list)

    def __iter__(self):
        """
        An iterator to this frame's cell's neighbors.
        This method simply returns the frame itself, which is that iterator.
        """
        return self

    def __next__(self):
        """Yields the next neighbor of this frame's cell."""
        return next(self._nei_iter)

    @property
    def junctions(self):
        """
        Gets the number of junctions while traversing to this frame's cell.
        If this frame's cell is a junction, the count includes it.
        """
