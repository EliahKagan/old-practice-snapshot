...
class Board {
    ...
    countChoices() {
        return this._canGo(this._iStart, this._jStart) ? this._dfs() : NO_PATH;
    }
    ...
    _dfs() {
        ...
        const push = (i, j) => {
            const nei = [[i + 1, j],
                         [i - 1, j],
                         [i, j + 1],
                         [i, j - 1]].filter(coords => this._canGo(...coords));

            stack.push(new Board.Frame(i, j, nei));
            this._cells[i][j] = true;
        };

        for (push(this._iStart, this._jStart); stack.length !== 0; ) {
            const frame = stack[stack.length - 1];

            if (frame.nei.length === 0) {
                this._cells[frame.i][frame.j] = false;
                stack.pop();
                continue;
            }

            const [i, j] = frame.nei.pop();

            if (i === this._iEnd && j === this._jEnd) {
                return stack.reduce((a, e) => a + e.branch, 0);
            }
...
Board.Frame = function(i, j, nei) {
    this.i = i;
    this.j = j;
    this.nei = nei;
    this.branch = nei.length > 1;
};
