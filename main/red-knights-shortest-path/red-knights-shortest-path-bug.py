...
while queue:
    ...
    for dest_i, dest_j in neighbors(src_i, src_j):
        ...
        if dest_i == end_i and dest_j == end_j:
            print("Got to the end.")  # FIXME: remove after debugging
            break
    ...
    assert not queue
    assert not (dest_i == end_i and dest_j == end_j)
    #print(grid)  # FIXME: remove after debugging
    assert grid[end_i][end_j] is None
...
assert grid[end_i][end_j] is not None
