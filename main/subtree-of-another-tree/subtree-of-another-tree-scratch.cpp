...
bool Solution::isSubtree(const TreeNode* const s, const TreeNode* const t)
{
    ...
    createIds = [&ids, &createIds](const TreeNode* const root) {
        if (root == nullptr) return null_id;
        
        const Key key {root->val, createIds(root->left),
                                  createIds(root->right)};
    ...
    const function<size_t(const TreeNode*)>
    getId = [&ids, &getId](const TreeNode* const root) {
        if (root == nullptr) return null_id;

        const auto left_id = getId(root->left);
        if (left_id == no_id) return no_id;

        const auto right_id = getId(root->right);
        if (right_id == no_id) return no_id;

        const auto p = ids.find(Key{root->val, left_id, right_id});

        //const auto p = ids.find(Key{root->val, getId(root->left),
        //                                       getId(root->right)});
