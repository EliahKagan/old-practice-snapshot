...
namespace {
    ...
    constexpr bool have_contiguous_alphabet(const char *const p = alphabet,
                                            const std::ptrdiff_t i = 0)
    {
        return *p == '\0' || (static_cast<std::ptrdiff_t>(*p - 'A') == i
                              && have_contiguous_alphabet(p + 1, i + 1));
    }

    static_assert(have_contiguous_alphabet(),
                  "don't have contiguous upper-case letters");
    ...
    constexpr auto k_piece_mask = (1u << base(Player::black)) - 1u;
    ...
    {
        return base(obj) & (k_piece_mask << base(player));
    }
...
#pragma GCC diagnostic pop
#endif
    public:
        Game(Board& board, int moves_including_current,
    ...
    private:
        ...
        // helper functions for try_pawn_moves()
        inline bool pawn_can_capture(size_t ii, size_t jj) const;
        inline bool try_promotions(size_t i, size_t j, size_t ii, size_t jj);
        ...
        bool try_move(const size_t i, const size_t j,
                      const size_t ii, const size_t jj,
                      const Piece becomes_piece)
        {
            return try_move(i, j, ii, jj, as_object(becomes_piece, player_));
        }
    ...
    Game::Game(Board& board, const int moves_including_current,
               const Player player, const bool draw_wins)
        : board_(board), moves_{moves_including_current - 1},
          player_{player}, draw_wins_{draw_wins}
    {
        if (moves_ < 0)
    ...
    bool Game::has_winning_strategy()
    {
        for (size_t i {0u}; i != k_board_size; ++i) {
            for (size_t j {0u}; j != k_board_size; ++j) {
                ...
                assert(false && "unrecognized piece on board (?!)");
            }
        }
    ...
    {
        // deltas for diagonal movement while capturing
        static constexpr std::array<size_t, 2> djs {{static_cast<size_t>(-1),
                                                     static_cast<size_t>(+1)}};

        // this is always in range, or the pawn would no longer be a pawn
        const auto ii = i + pawn_direction(player_);

        if (ii == promotes_at_row(player_)) {
            if (board_[ii][j] == Object::empty && try_promotions(i, j, ii, j))
                return true;                        // we must promote

            for (const auto dj : djs) {
                if (j + dj < k_board_size && pawn_can_capture(ii, j + dj)
                                          && try_promotions(i, j, ii, j + dj))
                    return true;                    // we must promote
            }
        } else {
            if (board_[ii][j] == Object::empty && try_move(i, j, ii, j))
                return true;                        // we must remain a pawn

            for (const auto dj : djs) {
                if (j + dj < k_board_size && pawn_can_capture(ii, j + dj)
                                          && try_move(i, j, ii, j + dj))
                    return true;                    // we must remain a pawn
            }
        }
    ...
    inline bool Game::pawn_can_capture(const size_t ii, const size_t jj) const
    {
        return jj < k_board_size
                && belongs_to(opponent(player_), board_[ii][jj]);
    }

    inline bool Game::try_promotions(const size_t i, const size_t j,
                                     const size_t ii, const size_t jj)
    {
        return try_move(i, j, ii, jj, Piece::rook)
            || try_move(i, j, ii, jj, Piece::knight)
            || try_move(i, j, ii, jj, Piece::bishop);
    }
...
int main()
{
    ...
    auto g = 0;
    for (std::cin >> g; g > 0; --g) {
        auto white_count = 0, black_count = 0, moves_including_current = 0;
        std::cin >> white_count >> black_count >> moves_including_current;
        auto board = read_initial_board(white_count, black_count);

        Game game {board, moves_including_current};
        std::cout << (game.has_winning_strategy() ? "YES" : "NO") << '\n';
    }
}
