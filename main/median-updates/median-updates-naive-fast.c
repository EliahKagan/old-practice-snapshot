...
enum debugging_options { // but remember NDEBUG may be defined (like on line 1)
    k_nice_debugging = 0,
    k_assert_no_dupes = 0 // if nonzero, ensures the multiset is used as a set
};

enum special_values {
    k_not_found = -1,   // returned instead of an index - don't change this
    k_no_value = 0,     // "doesn't mean to hold a value yet"
    k_nmax = 100000
};
...
static int vec_rindex_of(const struct vec *const vp, const Element e)
{
    assert(0 <= vp->length && vp->length <= vp->capacity);
    assert((vp->capacity == 0) == (vp->elems == NULL));

    int i = vp->length;
    while (i-- != 0 && vp->elems[i] != e) { }
    return i; // returns -1 (k_not_found) if the element is not found
}
...
// AD-HOC MULTISET DATA STRUCTURE:
//
// (Not a full multiset implementation. The "hash" of every element is itself.
// Buckets are vecs, so pointers into them would not survive resizing of them.
// The number of buckets is set upon construction, never changes, and taken as
// a separate function parameter. This data structure is designed specifically
// for the problem at hand, is much simpler than a "real" set or multiset, and
// is ad-hoc in the sense that some important operations must be performed by
// the caller. I use this only as a set; it is implemented as a multiset so
// that, when assertions are disabled, the processing, and code complexity,
// associated with checking for uniqueness are elided.)

#ifdef NDEBUG
ATTRIBUTE (( const ))
#endif
ATTRIBUTE (( warn_unused_result ))
static inline int multiset_guess_best_bucket_count(const int n)
{
    // TODO: Check if ln(2) [https://stackoverflow.com/a/31401836] is really
    // the optimal load factor for this application, by doing some benchmarks.
    assert(0 < n); // the anticipated load should be strictly positive
    return (int)lroundf((float)n / logf(2.0f));
}

ATTRIBUTE (( malloc, returns_nonnull, warn_unused_result ))
static inline struct vec *multiset_create(const int slen) // pass bucket count
{
    assert(0 < slen); // even a degenerate multiset needs a bucket
    return xcalloc((size_t)slen, sizeof *multiset_create(slen));
}

ATTRIBUTE (( nonnull ))
static void multiset_destroy(const int slen, struct vec *restrict *const sp)
{
    assert(0 < slen);
    assert(*sp != NULL);
...
*multiset_get_bucket(const int slen, struct vec *const s, const Element e)
{
...
ATTRIBUTE (( nonnull ))
static inline void multiset_insert(const int slen, struct vec *const s,
                                   const Element e)
{
    struct vec *const bp = multiset_get_bucket(slen, s, e);
    assert(k_assert_no_dupes == (0) || vec_rindex_of(bp, e) == k_not_found);
    vec_append(bp, e);
}
...
ATTRIBUTE (( nonnull ))
static inline void
multiset_erase_from_bucket(struct vec *const bp, const int bi, const Element e)
{
#ifdef NDEBUG
    (void)e; // unused except for assertions
#endif
    ...
    assert(vec_rindex_of(bp, e) == k_not_found);
}
                       ...
                       const int slen, struct vec *restrict const s)
{
    ...
    for (int i = 0; n-- != 0; ) { // i indexes that last element in a
        ...
        if (q == 'a') {
            ...
            // If a did not already contain an e before this, add e to s.
            if (a[j] != e || j == 0) multiset_insert(slen, s, e);
        } else {
            // This query performs REMOVAL, so check s to see if a has an e.
            struct vec *const bp = multiset_get_bucket(slen, s, e);
            const int bi = vec_rindex_of(bp, e);
            if (bi == k_not_found) {
no_update:
                puts("Wrong!");
                continue;
            ...
            if (a[j] != e || j == 0) multiset_erase_from_bucket(bp, bi, e);
...
int main(void)
{
    ...
    const int slen = multiset_guess_best_bucket_count(n);
    struct vec *restrict s = multiset_create(slen);
    ...
    if (k_nice_debugging != (0)) multiset_destroy(slen, &s);
}
