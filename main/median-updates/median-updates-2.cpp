...
namespace {
    ...
    double MedianBag::median() const noexcept
    {
        ...
        case 0: // even number of values
            ...
            return (static_cast<double>(*cbegin(low_))
                    + static_cast<double>(*cbegin(high_))) / 2.0;

        case -1: // odd number of values, with the middle in the low values
            return static_cast<double>(*cbegin(low_));

        case +1: // odd number of values, with the middle in the high values
            return static_cast<double>(*cbegin(high_));
