...
namespace {
    ...
    constexpr bool square_is_in_bounds(const unsigned long long nmax)
    {
        return nmax * nmax < std::numeric_limits<Index>::max();
    }
...
int main()
{
    ...
    static_assert(square_is_in_bounds(nmax), "nmax too big (32-bit build?)");
