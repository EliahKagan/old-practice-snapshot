...
class Node
  ...
  def require_sentinel
    raise 'invalid operation on non-sentinel node' unless @frequency.zero?
  end
end
...
class LFUCache
  ...
  private def access(node, key, value = nil)
    ...
    if node
      ...
      # @table.delete(@sentinel.drop) if @table.size > @capacity
      print "(#{@table.size}) "
      if @table.size > @capacity
        dropped = @sentinel.drop
        raise 'Internal error: deleting the new node!' if dropped == node
