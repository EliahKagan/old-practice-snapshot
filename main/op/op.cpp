...
// make Long a platform/implementation-specific reasonably long integer
...
_Noreturn inline void quit(); // explicitly flushes cout to throw on error
...
_Noreturn inline void quit() // first flushes cout explicitly to throw on error
{
    std::cout << std::flush;
...
bool line(std::string& op, std::vector<double>& operands) // true on success
{
    ...
    if (!(linestream >> op)) quit(); // blank line exits
