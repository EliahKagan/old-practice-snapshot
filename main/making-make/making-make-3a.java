...
final class Graph {
    ...
    List<Integer> topologicalSort() {
        ...
        for (final Deque<Integer> roots = getRoots(); !roots.isEmpty(); ) {
            final int src = roots.pop();

            for (final int dest : adj.get(src))
                if (--inorders[dest] == 0) roots.push(dest);
    ...
    private Deque<Integer> getRoots() {
        final Deque<Integer> roots = new ArrayDeque<>();

        for (int src = 0; src != adj.size(); ++src)
            if (inorders[src] == 0) roots.push(src);
