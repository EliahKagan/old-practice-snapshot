...
int sum(int count, ...)
{
    ...
    va_start(args, count);
    for (assert(count >= 0); count > 0; --count) acc += va_arg(args, int);
    va_end(args);

    return acc;
}

static inline int min_of(const int x, const int y)
{
    return y < x ? y : x;
}
...
int min(int count, ...)
{
    ...
    for (assert(count > 0); count > 0; --count)
        acc = min_of(acc, va_arg(args, int));
    va_end(args);

    return acc;
}

static inline int max_of(const int x, const int y)
{
    return y > x ? y : x;
}
...
int max(int count, ...)
{
    ...
    va_start(args, count);
    for (assert(count > 0); count > 0; --count)
        acc = max_of(acc, va_arg(args, int));
    va_end(args);
