...
static int fold(int acc, int (*const func)(int, int), int count, va_list args)
{
    assert(count > 0);
    while (count-- > 0) acc = func(acc, va_arg(args, int));
    return acc;
}
...
int sum(const int count, ...)
{
    va_list args;
    va_start(args, count);
    const int ret = fold(0, add, count, args);
    va_end(args);
    return ret;
}

static int min_of(const int x, const int y)
{
    return y < x ? y : x;
}
...
int min(const int count, ...)
{
    va_list args;
    va_start(args, count);
    const int ret = fold(INT_MAX, min_of, count, args);
    va_end(args);
    return ret;
}

static int max_of(const int x, const int y)
{
    return y > x ? y : x;
}
...
int max(const int count, ...)
{
    va_list args;
    va_start(args, count);
    const int ret = fold(INT_MIN, max_of, count, args);
    va_end(args);
    return ret;
}
