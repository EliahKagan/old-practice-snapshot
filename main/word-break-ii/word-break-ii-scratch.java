...
class Solution {
    ...
    pathAsString(final String text, final List<Integer> path) {
        ...
        for (int k = 1; k != path.size(); ++k)
            sj.add(text.substring(path.get(k - 1), path.get(k)));
...
final class UnitTest {
    private static void
    test(final String text, final String concatenatedWords) {
        System.out.format("%ntest(\"%s\", \"%s\")%n%n",
                          text, concatenatedWords);

        final List<String> words = List.of(concatenatedWords.split("\\s+"));

        for (final String path : new Solution().wordBreak(text, words))
