...
namespace {
    ...
    {
        // similar strings have a first mismatch
        tie(p, q) = mismatch(p, pp, q);
        if (p == pp) return false;
        
        // similar strings have a second mismatch, corresponding to the first
        const auto x = *p, y = *q;
        tie(p, q) = mismatch(++p, pp, ++q);
...
int main()
{
    ...
    const vector<string> a {"tars", "rats", "arts", "star"};
    const vector<string> b {"abcdef", "defacb", "abcfde", "abcfed", "fedcba"};
    const vector<string> c {"abcdef", "defacb", "abcfde", "fedcba"};

    Solution s;
    cout << s.numSimilarGroups(a) << '\n'
         << s.numSimilarGroups(b) << '\n'
         << s.numSimilarGroups(c) << '\n';
}
