...
internal static class Solution {
                                     ...
                                     int i, int j, int imax, int jmax)
    {
        ...
        if (i != 0)     area += grid.MeasureRegion(i - 1, j, imax, jmax);
        if (i != imax)  area += grid.MeasureRegion(i + 1, j, imax, jmax);
        if (j != 0)     area += grid.MeasureRegion(i, j - 1, imax, jmax);
        if (j != jmax)  area += grid.MeasureRegion(i, j + 1, imax, jmax);
