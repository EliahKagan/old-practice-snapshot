...
// Assumes every node's indegree is equal to its outdegree, and checks if the
// nodes of nonzero in degree are a strongly component by performing a depth-
// first search from one of them.
bool Graph::isSC()
{
    ...
    array<bool, Vmax> vis {};
    auto w = V; // a node that another can reach, or V if there are none

    for (auto v = 0; v != V; ++v) {
        if (in[v] == 0) vis[v] = true;
        else w = v;
    }
