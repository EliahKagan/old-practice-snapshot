...
internal sealed class PriorityQueue<T> {
    ...
    public T Pop() {
        ...
        this.Dump($"Removed {ret}. About to heapify down.");
        HeapifyDown();
    ...
    private void HeapifyUp() {
        for (var current = Count - 1; current > 0; ) {
            ...
            if (AlreadyOrdered(parent, current)) break;
            Swap(parent, current);
            current = parent;
        }
    }
    ...
    private void HeapifyDown() {
        for (var current = 0; ; ) {
            ...
            int child;
            if (AlreadyOrdered(current, leftChild)) {
                var rightChild = leftChild + 1;
                if (rightChild >= Count || AlreadyOrdered(current, rightChild))
                    break;
    ...
    private bool AlreadyOrdered(int i, int j)
        => _compare(_elems[i], _elems[j]) <= 0;
...
public class MedianFinder {    
    public void AddNum(int num) {
        ...
        _low.Dump($"Maxheap (_low) after {num}");
        _high.Dump($"Minheap (_high) after {num}");
        Rebalance();
        _low.Dump($"Maxheap (_low) after {num}, rebalanced");
        _high.Dump($"Minheap (_high) after {num}, rebalanced");
    }
