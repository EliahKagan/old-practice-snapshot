...
class Solution {
    ...
    buildAdjacencyList(final int numCourses, final int[][] prerequisites) {
        ...
        for (final int[] edge : prerequisites) adj.get(edge[1]).add(edge[0]);
...
final class UnitTest {
    ...
    private static void shouldBeTrue() {
        final int numCourses = 6;

        final int[][] prerequisites = {
                {0,1}, {0,2}, {1,2}, {2,3}, {1,5}, {2,5}, {4,0}, {4,5}
        };

        test(numCourses, prerequisites);
    }

    private static void shouldBeFalse() {
        final int numCourses = 6;

        final int[][] prerequisites = {
                {0,1}, {0,2}, {1,2}, {2,3}, {1,5}, {2,5}, {4,0}, {4,5}, {3,4}
        };

        test(numCourses, prerequisites);
    }

    private static void test(final int numCourses,
                             final int[][] prerequisites) {
        System.out.format("canFinish(%d, %s) -> %b%n",
