...
public class Solution {
    public int[] HitBricks(int[][] grid, int[][] hits)
    {
        ...
        for (var pos = hits.Length - 1; pos != -1; --pos) {
            if (hits[pos] == null) continue; // we add the cell earlier instead
            
            Add(grid, sets, hits[pos][0], hits[pos][1]);
            var newReach = sets.ReachableCount;
            if (newReach == reach) continue; // the root can't reach the cell
            
            if (newReach < reach) {
                throw new InvalidOperationException(
                        "Internal error: connectivity increases over time!");
            }
                            ...
                            int i, int j)
    {
        if (grid[i][j] != 0) {
            throw new InvalidOperationException(
                    "Internal error: \"deleted\" vertex was reachable!");
        }
    ...
    {
        if (IndexOk(destRow, sets.Height) && IndexOk(destCol, sets.Width)
                                          && grid[destRow][destCol] != 0)
            sets.Connect(srcRow, srcCol, destRow, destCol);
    }
    
    private static bool IndexOk(int index, int exclusiveUpperBound)
        => 0 <= index && index < exclusiveUpperBound;
}
...
internal static class UnitTest {
    private static void Main()
    {
        ...
        var hits3 = new[] { new[] { 3, 0 },
                            new[] { 4, 0 },
                            new[] { 1, 0 },
                            new[] { 2, 0 },
                            new[] { 0, 0 } };

        s.HitBricks(grid3, hits3).Dump("3");
    }
}
