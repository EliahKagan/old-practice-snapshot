...
static ptrdiff_t lower_bound(const uint32_t *const a, const uint32_t key,
                             const ptrdiff_t low, const ptrdiff_t high)
{
    if (high < low) return low;

    const ptrdiff_t mid = low + (high - low) / 2;

    return a[mid] < key ? lower_bound(a, key, mid + 1, high)
                        : lower_bound(a, key, low, mid - 1);
}
...
ATTRIBUTE ((nonnull, pure))
static bool has_small_factor(const uint32_t *primes, const ptrdiff_t size,
                             const uint32_t n)
{
    assert(size > 0);
...
ATTRIBUTE ((nonnull, pure))
static bool is_prime(const uint32_t *const primes, const ptrdiff_t size,
                     const uint32_t n)
{
    assert(size > 0);
    
    const ptrdiff_t i = lower_bound(primes, n, 0, size - 1);
...
int main(void)
{
    ...
    for (scanf("%d", &t); t > 0; --t) {
        uint32_t n = 0;
        (void)scanf("%" SCNu32, &n);
        puts(is_prime(primes, size, n) ? "PRIME" : "NOT PRIME");
    }
