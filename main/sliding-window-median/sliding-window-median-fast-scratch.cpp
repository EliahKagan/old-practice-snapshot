...
namespace {
    constexpr auto k_debug_bagops = false;

    class MedianBag {
        friend ostream& operator<<(ostream&, const MedianBag&);
    ...
    {
        if (k_debug_bagops) cerr << "insert(" << e << ") -> ";
        ...
        if (k_debug_bagops) cerr << *this << '\n';
    }

    bool MedianBag::erase(const int e) noexcept
    {
        if (k_debug_bagops) cerr << "erase(" << e << ") -> ";

        const auto p = bag_.lower_bound(e);
        if (p == end(bag_) || *p != e) {
            if (k_debug_bagops) cerr << "not found, no change\n";
            return false;
        }
    ...
    ostream& operator<<(ostream& out, const MedianBag& bag)
    {
        out << "(( ";

        for (auto p = begin(bag.bag_); p != end(bag.bag_); ++p) {
            if (p == bag.pos_) out << '[' << *p << "] ";
            else out << *p << ' ';
        }
