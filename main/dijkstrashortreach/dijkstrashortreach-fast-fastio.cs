...
internal static class Parser {
    internal static int Get() => Items[Pos++];

    private static readonly int[] Items =
            Array.ConvertAll(Console.In.ReadToEnd()
                                .Split((char[])null,
...
internal static class Solution {
    ...
    {
        for (var t = Parser.Get(); t > 0; --t) {
            var adj = ReadGraph();
            var start = Parser.Get();
    ...
    {
        var vertexCount = Parser.Get();
        var edgeCount = Parser.Get();
