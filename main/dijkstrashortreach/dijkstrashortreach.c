...
static int *dijkstra(const struct graph *const gp, const int start)
{
    int *const costs = xcalloc(gp->vertex_count, sizeof *costs);
    fill(costs, gp->vertex_count, k_no_path);

    struct heap h = heap_init(gp->vertex_count);
    for (heap_emplace_or_decrease(&h, start, 0); !heap_empty(&h); ) {
        ...
        for (const struct edge *const ep_end = vec_end(rowp),
            ...
            if (costs[ep->dest] == k_no_path)
                heap_emplace_or_decrease(&h, ep->dest, cost + ep->weight);
        }
    }
...
static void print_costs(const int *const costs, const int vertex_count,
                        const int start)
{
    const char *sep = "";
...
int main(void)
{
    ...
    for ((void)scanf("%d", &t); t > 0; --t) {
        ...
        int *costs = dijkstra(&g, start);
        print_costs(costs, g.vertex_count, start);
