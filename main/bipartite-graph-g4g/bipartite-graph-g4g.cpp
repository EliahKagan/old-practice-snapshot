#include "bipartite-graph.h" //#include <bits/stdc++.h>
using namespace std;
#define MAX 100
...
namespace {
    ...
    // Disconnects every vertex from itself.
    void removeLoops(int (*const g)[MAX], const int v)
    {
        for (auto i = 0; i != v; ++i) g[i][i] = 0;
    }
...
bool isBipartite(int (*const g)[MAX], int v)
{
    ...
    makeUndirected(g, v);
    removeLoops(g, v); // an attempt to adapt to wrong G4G test cases
