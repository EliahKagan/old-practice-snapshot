...
class Heap {
    ...
    push(value) {
        this._siftUp(this.length, value);
    }
    ...
    pop() {
        ...
        if (this.length === 1) {
            ...
            this._siftDown(0, this._elems.pop());
        }
    ...
    _siftUp(child, value) {
        while (child !== 0) {
            ...
            if (this._compare(this._elems[parent], value) <= 0) break;

            this._elems[child] = this._elems[parent];
            child = parent;
        }

        this._elems[child] = value;
    }

    _siftDown(parent, value) {
        for (; ; ) {
            const child = this._pickChild(parent);
            if (child === undefined ||
                    this._compare(value, this._elems[child]) <= 0) {
                break;
            }
    ...
    _pickChild(parent) {
        ...
        if (this._compare(this._elems[left], this._elems[right]) <= 0) {
            return left;
        }
