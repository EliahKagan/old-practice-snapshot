...
internal static class Solution {
    ...
    private static void Add(MaxHeap low, MinHeap high, int e)
    {
        if (e < low.Top) low.Add(e);
        else high.Add(e);
    }

    private static void Rebalance(MaxHeap low, MinHeap high)
    {
    ...
    private static double ComputeMedian(MaxHeap low, MinHeap high)
    {
        Console.Error.WriteLine(
            $"low.Count = {low.Count}, high.Count = {high.Count}");
        Console.Error.WriteLine(
            $"low.Top = {low.Top}, high.Top = {high.Top}");

        return low.Count == high.Count
                ? (low.Top + high.Top) / 2.0
                : (low.Count > high.Count ? low.Top : high.Top);
    }
    ...
    private static void Main()
    {
        ...
        while (--n != 0) {
            Add(low, high, Get());
            Rebalance(low, high);
            Console.WriteLine("{0:F1}", ComputeMedian(low, high));
        }
    }
}
