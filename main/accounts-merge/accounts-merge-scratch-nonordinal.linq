...
internal sealed class DisjointSets<T> : IEnumerable<IOrderedEnumerable<T>>,
    ...
    public IEnumerator<IOrderedEnumerable<T>> GetEnumerator()
        => _elems.GroupBy(kv => kv.Value.FindSet())
                 .Select(group => group.Select(kv => kv.Key).OrderBy(x => x))
                 .GetEnumerator();
    ...
    void ICollection<IEnumerable<T>>.Add(IEnumerable<T> subset) => Add(subset);
    
    bool ICollection<IEnumerable<T>>.Contains(IEnumerable<T> subset)
        => throw new NotSupportedException(
                "exact subset query is unsupported");
    
    void ICollection<IEnumerable<T>>.CopyTo(IEnumerable<T>[] array, int index)
        => throw new NotSupportedException("copying to array is unsuppported");
    
    bool ICollection<IEnumerable<T>>.Remove(IEnumerable<T> subset)
        => throw new NotSupportedException(
...
public class Solution {
    public IList<IList<string>> AccountsMerge(IList<IList<string>> accounts) {
        ...
        var sets = new DisjointSets<string>();
        
        foreach (var group in accounts.GroupBy(Account.Name)) {
            var name = new[] { group.Key };
            foreach (var account in group) sets.Add(account.Emails());
