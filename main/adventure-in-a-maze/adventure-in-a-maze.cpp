...
namespace {
    ...
    void Game::populate_everything_else()
    {
        for (std::size_t i {1u}; i != size_; ++i) {
            for (std::size_t j {1u}; j != size_; ++j) {
                const auto reachable_from_above =
                        reachable_[i - 1u][j] && maze_[i - 1u][j] != right_only;
                const auto reachable_from_left =
                        reachable_[i][j - 1u] && maze_[i][j - 1u] != down_only;

                if (!(reachable_from_above || reachable_from_left)) continue;

                reachable_[i][j] = true;

                if (reachable_from_above) {
                    paths_[i][j] += paths_[i - 1u][j];
                    adventure_[i][j] = std::max(adventure_[i][j],
                                                adventure_[i - 1u][j]);
                }

                if (reachable_from_left) {
