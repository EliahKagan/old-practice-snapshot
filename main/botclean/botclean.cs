...
internal static class Solution {
    ...
    // For broader testing possibilities, ReadBoard() accepts any rectangular
    // board, not just 5x5 as given in the problem description.
    private static IList<char[]> ReadBoard(out int m, out int n)
    {
        ...
        for (; ; ) {
            ...
            if (row == null || (row = row.TrimEnd()).Length == 0) break;
                                 ...
                                 ref int i, ref int j)
    {
        ...
        for (var d = 1; d <= dmax; ++d) {
            ...
            foreach (var p in DirtyPointsAtDistance(board, d, i, j, m, n)) {
                ...
                else Contract.Assert(false, "can't go where we already are");
